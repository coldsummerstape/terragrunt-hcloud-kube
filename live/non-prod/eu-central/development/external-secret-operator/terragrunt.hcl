...
include "envcommon" {
  path   = "${dirname(find_in_parent_folders())}/_envcommon/external-secret-operator.hcl"
  # We want to reference variables from the included configuration in this configuration, so we expose them.
  expose = true
}

terraform {
  source = "${dirname(find_in_parent_folders())}/modules/external-secret-operator"
}
...
inputs = {
  ...
  values_file = "values-external-secrets.yaml"
