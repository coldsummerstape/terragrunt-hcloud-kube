...
include "envcommon" {
  path   = "${dirname(find_in_parent_folders())}/_envcommon/external-dns-hetzner.hcl"
  # We want to reference variables from the included configuration in this configuration, so we expose them.
  expose = true
}

terraform {
  source = "${dirname(find_in_parent_folders())}/modules/external-dns-hetzner"
}
...
inputs = {
  ...
  values_file = "external-dns-hetzner-values.yaml"
