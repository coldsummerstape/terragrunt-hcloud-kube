...
include "envcommon" {
  path   = "${dirname(find_in_parent_folders())}/_envcommon/cert-manager-webhook-hetzner.hcl"
  # We want to reference variables from the included configuration in this configuration, so we expose them.
  expose = true
}

terraform {
  source = "${dirname(find_in_parent_folders())}/modules/cert-manager-webhook-hetzner"
}

# ---------------------------------------------------------------------------------------------------------------------
# MODULE PARAMETERS SPECIFIC TO THE ENVIRONMENT
# ---------------------------------------------------------------------------------------------------------------------
...
inputs = {
  ...
  count  = 1
  values_file = "cert-manager-webhook-hetzner-values.yaml"
  depends_on = ["../cluster"]
}